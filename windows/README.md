...
```powershell
New-Angular myapp              # menu interactif (base ou Angular Material)
...
Une seule **bibliothèque UI/UX** est incluse : **Angular Material** (la bibliothèque officielle d'Angular, avec Material 3 et le thème azure/blue). Les raccourcis sont organisés dans un **point d'entrée unique** : une seule ligne suffit dans votre profil PowerShell.
...
```powershell
New-Angular myapp              # scaffolde un projet (menu interactif 1-2)
New-AngularBase myapp          # template Angular + SCSS + Routing de base
