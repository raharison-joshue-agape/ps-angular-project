...
### Scaffold des projets **Angular + Angular Material / PrimeNG / ng-zorro-antd + TypeScript** pré-configurés directement depuis **Zsh**
...
`index.zsh` est le point d'entrée. Il source (`source`) les modules Angular Material, PrimeNG et ng-zorro-antd situés dans son propre répertoire (résolu via `${0:A:h}`), si bien que les raccourcis fonctionnent quel que soit l'endroit où le projet a été copié.
