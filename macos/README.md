...
### Scaffold des projets **Angular + Angular Material / PrimeNG + TypeScript** pré-configurés directement depuis **Zsh**
...
Deux **bibliothèques UI/UX** sont disponibles : **Angular Material** (la bibliothèque officielle d'Angular, avec Material 3 et le thème azure/blue) et **PrimeNG** (80+ composants enterprise, thème Aura). Les raccourcis sont organisés dans un **point d'entrée unique** : une seule ligne suffit dans votre `~/.zshrc`. La syntaxe est identique à celle des versions [Windows](../windows/README.md) et [Linux](../linux/README.md) (`new_angular` ⇄ `New-Angular`, `new_angular_primeng` ⇄ `New-AngularPrimeNg`).
...
`index.zsh` est le point d'entrée. Il source (`source`) les modules Angular Material et PrimeNG situés dans son propre répertoire (résolu via `${0:A:h}`), si bien que les raccourcis fonctionnent quel que soit l'endroit où le projet a été copié.
